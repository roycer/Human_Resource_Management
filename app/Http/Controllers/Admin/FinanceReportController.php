        ...
        foreach($invoices as $chart) {
            if($chart->currency_code != $this->global->currency->currency_code){
                if(isset($conversionRateArray['quotes'])){
                    $chartData[] = ['date' => $chart->date, 'total' => floor($chart->total / $conversionRateArray['quotes'][strtoupper($this->global->currency->currency_code.$chart->currency_code)])];
                }else{
                    $chartData[] = ['date' => $chart->date, 'total' => floor($chart->total)];
                }
