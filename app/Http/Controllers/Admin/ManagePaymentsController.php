...
class ManagePaymentsController extends AdminBaseController
{
    ...
    public function export($startDate, $endDate, $status, $project) {
        ...
        $payments = Payment::leftJoin('projects', 'projects.id', '=', 'payments.project_id')
            ->join('currencies', 'currencies.id', '=', 'payments.currency_id')
            ->select('payments.id', 'payments.amount', 'currencies.currency_symbol', 'currencies.currency_code', 'payments.status', 'payments.paid_on', 'payments.remarks');
        ...
        $attributes =  ['amount', 'currency_symbol', 'currency_code', 'paid_on'];
        ...
        $exportArray[] = ['ID','Status','Remark','Amount', 'Paid On'];
